...
<head>
<title>CDC Admin Workspace</title>
</head>
<body style="background-color: powderblue;">
	...
	<form id="regForm" method="post" action="cdc-delete.htm">
				...
				<table border="1">
					...
					<c:forEach items="${requestScope.users}" var="user">
						<tr>
							<td>${user.userId}</td>
							<td>${user.username}</td>
							<td>${user.person.fName}&nbsp ${user.person.lName}</td>
							<td>${user.role}</td>
							<td><input type="radio" id="delete"
								value="${user.userId}" name="delete"></td>
						</tr>
						<c:set var="count" value="${count+1}" scope="page" />
					</c:forEach>
				</table>
				<input id="number" type="hidden" name="number" value="${count}">
			</div>
		</div>
	...
	<script>
		...
		function updatepage() {
			...
			var del = document.getElementById("test").value;
			httpRequest.open("POST", "./cdc-delete.htm?delete=" + del, true);
			httpRequest.send();
		}
